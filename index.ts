const kontainerData = document.getElementById("kontainerData") as HTMLElement;
const search = document.getElementById('search') as HTMLInputElement;
const nilaiRata2 = document.getElementById('nilaiRata2') as HTMLElement;

type siswa = {name: string, class: string, score: number}
   const siswa: siswa[]  = [
        { name: "Alwahibi wafi", 
          class: "A", 
          score: 100 },
...
function renderData(dataSiswa: siswa[])  {
    const tampilanData = dataSiswa.map((item, index:number) => `
        <tr>
